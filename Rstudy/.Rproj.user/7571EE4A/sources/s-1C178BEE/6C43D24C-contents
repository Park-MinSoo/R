...
word_data <- readLines("book/애국가(가사).txt") # 줄 단위로 글을 읽어온다.
word_data

?extractNoun
word_data2 <- sapply(word_data, extractNoun, USE.NAMES = F) # extractNoun 명사만 뽑아낸다.
# USE.NAMES = F -> 컬럼명을 사용하지 않겠다.
word_data2
